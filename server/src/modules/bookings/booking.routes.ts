...
export function registerBookings(instance: FastifyInstance): void {
  ...
  app.get('/api/calendar', { schema: { querystring: CalendarQuery } }, async (request) => {
    try {
      return await service.calendar(request.query.from, request.query.to)
    } catch (error) {
      if (error instanceof AppError) throw error
      // An unreachable engine renders an error, never an empty calendar: an empty grid reads
      // as "everything is free", which is the one mistake that costs money here.
      translateEngineError(error)
    }
  })
    ...
    async (request) => {
      try {
        return await service.reschedule(
          request.params.id,
          request.body.check_in,
          request.body.check_out,
        )
      } catch (error) {
        if (error instanceof AppError) throw error
        translateEngineError(error)
  ...
  app.patch(
    ...
    async (request) => {
      try {
        return await service.amend(request.params.id, request.body)
      } catch (error) {
        if (error instanceof AppError) throw error
        translateEngineError(error)
      }
